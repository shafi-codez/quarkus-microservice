node {
	...
	stage('Create tar.gz') {
       sh 'tar -czvf cognitive-banter-microservice-chart.tar.gz -C helm cognitive-banter-microservice'
       archiveArtifacts artifacts: 'cognitive-banter-microservice-chart.tar.gz', fingerprint: true
	}
	stage('Push chart to S3') {
	    withAWS(region:'us-west-2', credentials:'spinnaker-admin-aws') {
          s3Upload(file:'cognitive-banter-microservice-chart.tar.gz', bucket:'helm-charts-f2bba284-98d3-445b-9f04-a08c57b7d36e', path:"${JOB_NAME}/${BUILD_ID}/cognitive-banter-microservice-chart.tar.gz")
        }
	}
